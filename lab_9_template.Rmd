...
#The relationships appear linear, can only look at linearity of continuous variables
...
#Notes : this is BAD (because pricepersqft is calculated from the depecdent variable price -- biased overfitting of model results)
#also not good because of colinearity (three coefficents very related)
# b. Model summary:
summary(homes_lm1)
```
...
homes_lm2
#what the coefficent of catigorical var means: If two hoses are exactly the same a city in SLO would sell for th 34525 dollars more than Arryo Grande (reference variable); catigorical variables always in the realation to the reference variable
...
#look at what the class is -> City is character
homes_sub$Status <- factor(homes_sub$Status) #making City a factor in R brain
...
#Variance look ok from Residual vs Fitted
#QQ-plot shows normal
#Assumuption of risiduals normally dist good

#Overall, heteroscedasticitu is Ok, residual normallity definetely looks good, and conceptually and mathematically my model is making sense!
...
#based on AIC alone you would pick our saturated model but ultimately that doesn't describe the best model because conceptually model 3 makes the best sense. I would still pick homes_lm3 because #Overall, heteroscedasticitu is Ok, residual normallity definetely looks good, and conceptually and mathematically my model is making sense!
...
df_new <- data.frame(City = rep(c("San Luis Obispo",
                     ...
                     SqFt = rep(seq(from = 500,
                                    to = 3500, 
                                    length = 20), #number of observations in the df
                                times = 12), 
...
price_predict <- predict(homes_lm3,
                         newdata = df_new,
                         se.fit = TRUE, #adds the standard error
                         interval = "confidence") #adds the confidence invterval
...
predict_graph
#predictions line up with the coefficent values
